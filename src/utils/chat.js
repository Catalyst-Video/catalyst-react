...
// New room hash not dependent on any url params
const coreUrl = window.location.origin + window.location.pathname;
const roomHash = coreUrl.substring(url.lastIndexOf("/") + 1).toLowerCase();
var mode = "camera";
...
if (urlParams.get(" ") === "true") headerLogo.fadeOut();
			...
			dataChannel.get(uuid).onmessage = function (event) {
				...
				if (dataType === "mes:") {
					handlereceiveMessage(
						cleanedMessage,
						hueToColor(VideoChat.peerColors.get(uuid))
					);
...
function rePositionCaptions() {
	console.log("Reposition captions is disabled...");
...
function openFullscreen() {
	try {
		...
		if (!isFullscreen) {
			VideoChat.remoteVideoWrapper.classList.add("fullscreen");
			isFullscreen = true;
			...
			isFullscreen = false;
			VideoChat.remoteVideoWrapper.classList.remove("fullscreen");
...
function handlereceiveMessage(msg, color) {
	// Add message to screen
...
function uuidToHue(uuid) {
	...
	VideoChat.peerColors.forEach(function (value, key, map) {
		availColors[Math.floor(value / 60)] = null;
	});
...
function setStreamColor(uuid) {
	...
	document.querySelectorAll(
		`[uuid="${uuid}"]`
	)[0].style.border = `3px solid ${hueToColor(hue)}`;
}
...
function displayWaitingCaption() {
	...
	captionText.text("Room ready. Waiting for others to join...").fadeIn();
...
function startUp() {
	...
	// Set tab title
	document.title = "Catalyst - " + coreUrl.substring(url.lastIndexOf("/") + 1);
	...
	captionText.draggable({ containment: "window" });
