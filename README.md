...
| `defaults`          | Settings for the default instance of Catalyst                                                                                   | ```{audioOn?: boolean; videoOn?: boolean; showChatArea?: boolean; showCaptionsArea?: boolean; showToastArea?: boolean; }```                              | ```{ audioOn: true, videoOn: true, showCaptionsArea: true,  showCaptionsArea: true, showToastArea: true }```  | Optional      |
| `hidden`          | Hide any of Catalyst's wide array of video options                                                    | ```{ mute?: boolean; pausevideo?: boolean; screenshare?: boolean; fullscreen?: boolean; chat?: boolean; captions?: boolean; endcall?: boolean;  }```                              |```{ mute: false, pausevideo: false, screenshare: false, chat: false, fullscreen: false, captions: false, endcall: false }```  | Optional      |
...
| `cstmToastMsg`  | Displays message in snackbar popup on session start | `Any element React can compile` or `string`                              | `Share your session key {sessionKey} with whoever wants to join `                                | Optional |
